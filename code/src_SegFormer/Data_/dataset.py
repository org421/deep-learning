"""
Dataset for Copy-Move Forgery Detection with SegFormer.

Simplified version without point prompts (SegFormer is a global segmenter).
"""
...
class ForgeryDataset(Dataset):
    ...
    Returns:
        - image: (C, H, W) float32 normalized with ImageNet mean/std
    ...
    def __init__(
        ...
        self.oversample_forged = oversample_forged and (mode == "train")

        # Load data by scanning directories
        self.samples = self._load_samples(max_images)

        # Oversample forged images to balance classes
        if self.oversample_forged:
            self.samples = self._apply_oversampling(self.samples)
    ...
    def _apply_oversampling(self, samples: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Oversample forged images so they match the authentic count."""
        forged = [s for s in samples if s['label'] == 1]
        authentic = [s for s in samples if s['label'] == 0]

        if not forged or not authentic:
            return samples

        # Repeat forged to match authentic count
        if len(forged) < len(authentic):
            repeats = len(authentic) // len(forged)
            remainder = len(authentic) % len(forged)
            oversampled_forged = forged * repeats + random.sample(forged, min(remainder, len(forged)))
            result = authentic + oversampled_forged
        else:
            result = samples

        random.shuffle(result)
        print(f"  -> Oversampling: {len(forged)} forged -> {len([s for s in result if s['label'] == 1])} "
              f"(balanced with {len(authentic)} authentic, total {len(result)})")
        return result
    ...
    def _build_noise_transforms(self) -> Optional[A.Compose]:
        """Build photometric augmentation transforms (forgery-aware)."""
    ...
    def __getitem__(self, idx: int):
        ...
        # Convert to tensors — keep [0, 255] range for model's internal normalization
        img_float = img.astype(np.float32)  # UCM-NetV2 preprocess handles ImageNet normalization
        img_tensor = torch.tensor(img_float.transpose(2, 0, 1), dtype=torch.float32)
