...
# =============================================================================
# Parallel oF1 worker (must be top-level for Windows multiprocessing)
# =============================================================================

def _compute_single_oF1(args):
    """
    Compute oF1 for a single image. Designed for multiprocessing.Pool.

    Args:
        args: tuple of (pred_mask_np, gt_mask_np, original_shape, threshold, min_area)

    Returns:
        dict with score and debug counters
    """
    pred_mask, gt_mask, orig_shape, threshold, min_area = args
    ...
    shape = orig_shape or pred_mask.shape

    pred_rle = convert_segmentation_to_submission(
        pred_mask, threshold=threshold, min_area=min_area, merge_distance=0
    )
    gt_rle = convert_segmentation_to_submission(
        gt_mask.astype(np.float32), threshold=0.5, min_area=1, merge_distance=0
    )

    pred_is_auth = (pred_rle == "authentic")
    gt_is_auth = (gt_rle == "authentic")

    result = {
        'pred_rle': pred_rle, 'gt_rle': gt_rle, 'shape': shape,
        'gt_auth': int(gt_is_auth), 'gt_forged': int(not gt_is_auth),
        'pred_auth': int(pred_is_auth), 'pred_forged': int(not pred_is_auth),
        'both_auth': 0, 'both_forged': 0,
        'gt_forged_pred_auth': 0, 'gt_auth_pred_forged': 0,
    }

    if pred_is_auth or gt_is_auth:
        if pred_is_auth and gt_is_auth:
            result['score'] = 1.0
            result['both_auth'] = 1
        else:
            result['score'] = 0.0
            if gt_is_auth and not pred_is_auth:
                result['gt_auth_pred_forged'] = 1
            else:
                result['gt_forged_pred_auth'] = 1
    else:
        result['both_forged'] = 1
        result['score'] = evaluate_single_image(pred_rle, gt_rle, shape)
