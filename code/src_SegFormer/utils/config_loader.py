"""
Configuration loader and manager for SegFormer training.
"""
...
def get_project_root() -> Path:
    ...
    Priority:
    1. SEGFORMER_ROOT environment variable (if set)
    2. Current working directory (CWD)
    """
    if 'SEGFORMER_ROOT' in os.environ:
        return Path(os.environ['SEGFORMER_ROOT'])
    return Path.cwd()
...
class ExperimentConfig:
    name: str = "segformer_b2_forgery_detection"
    seed: int = 42
...
class DataConfig:
    ...
    image_size: int = 512  # SegFormer input size
    num_workers: int = 4
...
@dataclass
class ModelConfig:
    # UCM-NetV2 variant (ucm_netv2_small, ucm_netv2_medium, ucm_netv2_large)
    variant: str = "ucm_netv2_small"
...
class SchedulerConfig:
    ...
    patience: int = 5
    min_lr: float = 1e-6
    # CosineAnnealingWarmRestarts params (legacy)
    T_0: int = 10
    T_mult: int = 2
    eta_min: float = 1e-6
...
class TrainingConfig:
    ...
    warmup_epochs: int = 0  # Linear LR warmup from lr/10 to lr
...
class LossConfig:
    ...
    # --- Forensic auxiliary losses ---
    edge_weight: float = 0.5        # Edge boundary supervision weight
    cls_weight: float = 0.3         # Image-level classification weight
    # --- Focal Tversky parameters ---
    tversky_alpha: float = 0.3      # FP weight (lower = tolerate more FP)
    tversky_beta: float = 0.7       # FN weight (higher = penalize missed forgeries)
    tversky_gamma: float = 0.75     # Focal exponent (lower = focus on hard examples)
    lambda_pred_score: float = 0.05
    ignore_label: int = -1
