"""Modele DinoV3-UperNet pour la detection de falsification. Encoder ViT + decoder UperNet."""
...
import sys
sys.path.insert(0, 'model')  # chemin vers le repo dinov3

try:
    # verifie si dinov3 est dispo
    import dinov3
...
class ConvModule(nn.Module):
    """Module convolution + GroupNorm + ReLU."""
        ...
        for scale in pool_scales:
            self.stages.append(
                nn.Sequential(
                    ...
                    ConvModule(in_channels, channels, kernel_size=1, padding=0, num_groups=num_groups),
                )
            )
        
        # bottleneck pour fusionner les features
        total_channels = in_channels + len(pool_scales) * channels
        self.bottleneck = ConvModule(total_channels, channels, kernel_size=3, padding=1, num_groups=num_groups)
...
class FPN(nn.Module):
    """Feature Pyramid Network pour la fusion multi-echelle."""
    ...
    def __init__(
        ...
        # connexions laterales (conv 1x1 pour reduire les canaux)
        self.lateral_convs = nn.ModuleList()
        for in_ch in in_channels_list:
            self.lateral_convs.append(
                ConvModule(in_ch, out_channels, kernel_size=1, padding=0, num_groups=num_groups)
            )
        
        # convolutions de sortie FPN (conv 3x3 apres fusion)
        self.fpn_convs = nn.ModuleList()
        for _ in in_channels_list:
            self.fpn_convs.append(
                ConvModule(out_channels, out_channels, kernel_size=3, padding=1, num_groups=num_groups)
            )
    
    def forward(self, features: List[torch.Tensor]) -> List[torch.Tensor]:
        """Passe avant du FPN."""
        # construction des laterales
        laterals = [conv(f) for conv, f in zip(self.lateral_convs, features)]
        
        # chemin descendant
        for i in range(len(laterals) - 1, 0, -1):
            # upsample et ajout au niveau inferieur
...
class UperNetDecoder(nn.Module):
    """Decoder UperNet avec PPM et FPN."""
    ...
    def __init__(
        ...
        # PPM sur la feature la plus profonde
        self.ppm = PPM(
        ...
        # FPN pour fusion multi-echelle
        # apres PPM, le premier canal devient ppm_channels
        fpn_in_channels = [ppm_channels] + encoder_channels[1:]
        self.fpn = FPN(
        ...
        # fusion de tous les niveaux FPN
        self.fpn_bottleneck = ConvModule(
            len(encoder_channels) * fpn_channels,
            fpn_channels,
            kernel_size=3,
            padding=1,
            num_groups=num_groups,
        ...
        self.segmentation_head = nn.Sequential(
            ConvModule(fpn_channels, fpn_channels // 2, kernel_size=3, padding=1, num_groups=num_groups),
            nn.Dropout2d(dropout_ratio) if dropout_ratio > 0 else nn.Identity(),
            nn.Conv2d(fpn_channels // 2, num_classes, kernel_size=1),
        )
        
        # tete de prediction IoU
        self.iou_head = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Flatten(),
            nn.Linear(fpn_channels, 64),
    ...
    def forward(
        ...
        """Passe avant du decoder UperNet."""
        # PPM sur la feature la plus profonde
        ppm_out = self.ppm(encoder_features[0])
        
        # remplace la feature la plus profonde par la sortie PPM pour le FPN
        fpn_inputs = [ppm_out] + encoder_features[1:]
        
        # sorties FPN
        fpn_outs = self.fpn(fpn_inputs)
        
        # upsample toutes les sorties FPN a la meme taille
        target_h, target_w = fpn_outs[-1].shape[2:]
        
        fpn_outs_resized = []
        for out in fpn_outs:
            if out.shape[2:] != (target_h, target_w):
                out = F.interpolate(out, size=(target_h, target_w), mode='bilinear', align_corners=False)
            fpn_outs_resized.append(out)
        
        # concatenation et fusion
        ...
        # sortie segmentation
        seg_logits = self.segmentation_head(fused)
        
        # upsample a la taille cible
...
class DinoV3Encoder(nn.Module):
    ...
    def __init__(
        ...
        # construction du backbone avec le code officiel DINOv3
    ...
    def _build_backbone(self, model_name: str, img_size: int) -> nn.Module:
        ...
        # import des fonctions hub officielles
        ...
        # creation du modele sans poids pre-entraines (on les charge separement)
    ...
    def _load_weights(self, weights_path: str):
        ...
        # gestion des differents formats de checkpoint
        if 'model' in state_dict:
            state_dict = state_dict['model']
        elif 'state_dict' in state_dict:
            state_dict = state_dict['state_dict']
        
        # supprime le prefixe si present
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        with torch.set_grad_enabled(not self.freeze):
            # extraction des couches intermediaires
        ...
        # ordre inverse (plus profond en premier pour le decoder)
        features = features[::-1]
...
class DinoV3UperNet(nn.Module):
    """Modele DinoV3 + UperNet pour la segmentation."""
    ...
    def __init__(
        ...
        self.mask_decoder = UperNetDecoder(
...
def build_dinov3_upernet(
    ...
    """Construit le modele complet DinoV3-UperNet."""
    model = DinoV3UperNet(
        backbone=backbone,
        weights_path=weights_path,
...
def load_checkpoint(model: DinoV3UperNet, checkpoint_path: str, device: str = "cuda") -> dict:
...
DinoV3UNet = DinoV3UperNet
build_dinov3_unet = build_dinov3_upernet
