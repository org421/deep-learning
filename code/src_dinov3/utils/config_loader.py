...
def get_project_root() -> Path:
    """Retourne le repertoire racine du projet."""
...
def load_config(config_path: str = None) -> Dict[str, Any]:
    ...
    if config_path is None:
        # On essaie d'abord le repertoire courant
        cwd_config = get_project_root() / "configs" / "config.yaml"
        if cwd_config.exists():
            config_path = cwd_config
        else:
            # Sinon chemin relatif au script
...
class ModelConfig:
    ...
    # Decodeur UNET (ancien)
    decoder_channels: List[int] = field(default_factory=lambda: [256, 128, 64, 32])
    
    # Parametres decodeur UperNet
    fpn_channels: int = 256        # Canaux de sortie FPN
    ppm_channels: int = 512        # Canaux bottleneck PPM
    dropout_ratio: float = 0.1     # Dropout dans le decodeur
    
    num_groups: int = 8  # Groupes GroupNorm (compatible accumulation de gradient)
    
    # Gel du backbone
    freeze_backbone: bool = True
    freeze_backbone_epochs: int = 5  # Nombre d'epochs avec backbone gele
    
    # Pour compatibilite avec SAFIRE
    num_pairs: int = 3
