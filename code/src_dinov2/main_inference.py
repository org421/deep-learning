"""
Script d'inférence automatisé pour DinoV2-UNet.
...
def load_model(checkpoint_path: str, config: Config, device: str) -> nn.Module:
    """Charge un modèle DinoV2-UNet depuis un checkpoint."""
    from networks.dinov2_unet_model import DinoV2UNet
    
    backbone = getattr(config.model, 'backbone', 'dinov2_vitb14')
    decoder_channels = getattr(config.model, 'decoder_channels', [256, 128, 64, 32])
...
def main():
    parser = argparse.ArgumentParser(description="Inférence DinoV2-UNet avec Pipeline Complète")
    ...
    print("="*80)
    print("🚀 INFÉRENCE DinoV2-UNet - Pipeline Complète")
    print("="*80)
    print("Pipeline:")
    ...
    report_lines.append("RAPPORT D'INFÉRENCE - DinoV2-UNet - Pipeline Complète")
    ...
    for exp_idx, exp_dir in enumerate(experiments):
        ...
        report_lines.append(f"Backbone: {getattr(config.model, 'backbone', 'dinov2_vitb14')}")
        report_lines.append(f"Decoder channels: {getattr(config.model, 'decoder_channels', [256, 128, 64, 32])}")
