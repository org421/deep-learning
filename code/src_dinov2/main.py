"""
Main Training Script for DinoV2-UNET Image Forgery Detection.

Simplified for UNet architecture (global segmenter, no point prompts).

Features:
- DinoV2 backbone with UNET decoder
...
from networks.dinov2_unet_model import DinoV2UNet, build_dinov2_unet, load_checkpoint
...
def create_model(config: Config, device_info: dict) -> nn.Module:
    """Create and setup DinoV2-UNET model."""
    model = build_dinov2_unet(
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='DinoV2-UNET Training and Inference')
