"""
Script d'inférence automatisé pour SAM2-UNet.
...
def load_model(checkpoint_path: str, config: Config, device: str) -> nn.Module:
    ...
    from networks.resnet_unet_model import ResNetUNet
    
    backbone = getattr(config.model, 'backbone', 'resnet50')
    decoder_channels = getattr(config.model, 'decoder_channels', [256, 128, 64, 32, 16])
...
def main():
    parser = argparse.ArgumentParser(description="Inférence SAM2-UNet avec Pipeline Complète")
    ...
    print("="*80)
    print("🚀 INFÉRENCE SAM2-UNet - Pipeline Complète")
    print("="*80)
    print("Pipeline:")
    ...
    report_lines.append("RAPPORT D'INFÉRENCE - Unet-RESTNET50 - Pipeline Complète")
    ...
    for exp_idx, exp_dir in enumerate(experiments):
        ...
        report_lines.append(f"Backbone: {getattr(config.model, 'backbone', 'resnet50')}")
        report_lines.append(f"Decoder channels: {getattr(config.model, 'decoder_channels', [256, 128, 64, 32, 16])}")
