"""
Main Training Script for ResNet-UNET Image Forgery Detection.

Simplified for UNet architecture (global segmenter, no point prompts).

Features:
- ResNet backbone (18, 34, 50, 101) with UNET decoder
...
from networks.resnet_unet_model import ResNetUNet, build_resnet_unet, load_checkpoint
...
def create_model(config: Config, device_info: dict) -> nn.Module:
    """Create and setup ResNet-UNET model."""
...
def update_optimizer_for_unfrozen_backbone(model: nn.Module, optimizer: torch.optim.Optimizer, config: Config):
    ...
    backbone_params = []
    for name, param in base_model.named_parameters():
        if 'unet.encoder' in name and param.requires_grad:
            backbone_params.append(param)
...
def main():
    ...
    if device_info['is_main']:
        ...
        print(f"  Model: ResNet-UNET ({config.model.backbone})")
    ...
    for epoch in range(start_epoch, config.training.num_epochs):
        ...
        if (epoch + 1) % config.training.val_every == 0:
            ...
            if device_info['is_main']:
                ...
                if ema_model is not None and ema_val_metrics is not None:
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='ResNet-UNET Training and Inference')
