...
def main():
    ...
    model = build_cmsegnet(
        pretrained_mobilenet=config.model.pretrained_mobilenet,
        pretrained_cmsegnet=config.model.pretrained_cmsegnet,
        device=str(device)
    )
    
    # Freeze backbone if requested
    freeze_backbone = getattr(config.training, 'freeze_backbone', False)
    unfreeze_epoch = getattr(config.training, 'unfreeze_epoch', 0)
    
    if freeze_backbone:
        model.freeze_encoder()
        print(f"Backbone (MobileNetV2) FROZEN")
        if unfreeze_epoch > 0:
            print(f"  -> Will unfreeze at epoch {unfreeze_epoch}")
    ...
    print(f"\nLoss configuration:")
    print(f"  BCE weight: {getattr(config.loss, 'bce_weight', 1.0)}")
    print(f"  Dice weight: {getattr(config.loss, 'dice_weight', 0.5)}")
    print(f"  Tversky weight: {getattr(config.loss, 'tversky_weight', 0.0)}")
    print(f"  pos_weight: {getattr(config.loss, 'pos_weight', 1.0)}")
    if getattr(config.loss, 'tversky_weight', 0.0) > 0:
        print(f"  Tversky alpha (FP): {getattr(config.loss, 'tversky_alpha', 0.3)}")
        print(f"  Tversky beta (FN): {getattr(config.loss, 'tversky_beta', 0.7)}")
    ...
    if config.training.use_ema:
        ema_model = EMAModel(model, decay=config.training.ema_decay, device=str(device))
    ...
    for epoch in range(1, config.training.num_epochs + 1):
        ...
        # Progressive unfreezing: unfreeze backbone at specified epoch
        if freeze_backbone and unfreeze_epoch > 0 and epoch == unfreeze_epoch:
        ...
        # Print epoch summary
        print(f"\n{'─'*70}")
        print(f"EPOCH {epoch:3d} | LR: {current_lr:.2e} | Grad: {train_metrics['gradient_norm']:.2f} -> {train_metrics['gradient_norm_clipped']:.2f} (clipped)")
        print(f"  Train   │ Loss: {train_metrics['train_loss']:.4f} | BCE: {train_metrics['train_bce']:.4f} | Dice: {train_metrics['train_dice']:.4f}")
        
        # Scheduler step (cosine only - plateau is stepped after validation)
        ...
        if epoch % config.training.val_every == 0:
            ...
            if ema_model is not None:
                # NETTOYAGE MÉMOIRE CRITIQUE AVANT VALIDATION EMA
                # Sans cela, les tensors résiduels de la 1ère validation peuvent causer OOM
                gc.collect()
                torch.cuda.empty_cache()
            ...
            print(f"  Val     │ Loss: {val_metrics['val_loss']:.4f} | oF1: {val_metrics['val_oF1']:.4f} | Dice: {val_metrics['val_dice']:.4f} | IoU: {val_metrics['val_iou']:.4f}")
            print(f"          │ Prec: {val_metrics['val_precision']:.4f} | Recall: {val_metrics['val_recall']:.4f} | forged_oF1: {val_metrics.get('val_forged_oF1', 0):.4f}")
            print(f"          │ GT: {val_metrics['debug_gt_forged']} forged, {val_metrics['debug_gt_authentic']} auth | Pred: {val_metrics['debug_pred_forged']} forged, {val_metrics['debug_pred_authentic']} auth")
            
            if ema_val_metrics is not None:
                print(f"  EMA     │ Loss: {ema_val_metrics['val_loss']:.4f} | oF1: {ema_val_metrics['val_oF1']:.4f}")
...
def run_inference(args):
    ...
    if 'metrics' in checkpoint:
        print(f"  oF1: {checkpoint['metrics'].get('val_oF1', 'N/A')}")
    ...
    metrics = validate(model, loader, criterion, device, config, epoch=0)
