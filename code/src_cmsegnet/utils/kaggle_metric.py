...
def mask_to_instances(mask: npt.NDArray, min_area: int = 10) -> List[npt.NDArray]:
    ...
    # Find connected components - use cv2 if available (much faster)
    try:
        import cv2
        num_labels, labeled_array, stats, _ = cv2.connectedComponentsWithStats(binary_mask, connectivity=8)
        num_features = num_labels - 1  # Exclude background
    except ImportError:
...
def merge_nearby_instances(instances: List[npt.NDArray], 
    ...
    # Use cv2 for dilation if available (faster)
    try:
        import cv2
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (distance_threshold*2+1, distance_threshold*2+1))
        def dilate_mask(m):
            return cv2.dilate(m, kernel, iterations=1)
    except ImportError:
        from scipy.ndimage import binary_dilation
        def dilate_mask(m):
            return binary_dilation(m, iterations=distance_threshold).astype(np.uint8)
...
def calculate_f1_score(pred_mask: npt.NDArray, gt_mask: npt.NDArray) -> float:
    ...
    OPTIMIZED: avoid unnecessary flatten, use direct numpy operations.
    ...
    Returns:
        F1 score (0 to 1)
    """
    # Direct computation without flatten (faster)
